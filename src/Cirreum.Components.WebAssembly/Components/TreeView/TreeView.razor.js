export function scrollNodeIntoView(treeview, treeviewNode, options = {}) {
    ...
    if (container && node) {
        ...
        if (relativeTop >= buffer && relativeBottom <= (visibleHeight - buffer)) {
            if (finalOptions.debug)
                console.log('Node is fully visible, no scrolling needed.');
            return;
        }
